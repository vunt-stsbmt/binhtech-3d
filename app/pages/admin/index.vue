  ...
  <main class="mx-auto max-w-7xl px-4 py-10 sm:px-6">
    <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div class="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 class="mt-2 text-3xl font-bold text-slate-900">Quản lý sản phẩm bintech 3D</h1>
      ...
      <article class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm xl:col-span-5">
        ...
        <form class="mt-5 grid gap-4" @submit.prevent="onSubmit">
          <div class="grid gap-4 md:grid-cols-2">
            <label class="text-sm">
              ...
              <input v-model.trim="form.name" required
                ...
                type="text" placeholder="Ví dụ: Vỏ cơ khí tùy chỉnh">
            </label>

            <label class="text-sm">
              <span class="mb-2 block text-slate-600">Tag công nghệ</span>
              <input v-model.trim="form.tag" required
                class="w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-slate-900 placeholder-slate-400 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/30"
                type="text" placeholder="Ví dụ: FDM">
            </label>
          </div>

          <label class="text-sm">
            <span class="mb-2 block text-slate-600">Mô tả</span>
            <textarea v-model.trim="form.description" required rows="3"
              class="w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-slate-900 placeholder-slate-400 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/30"
          ...
          <label class="text-sm">
            ...
            <input v-model.trim="form.image" required
              class="w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-slate-900 placeholder-slate-400 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/30"
              type="url" placeholder="https://placehold.co/800x560/...">
          </label>
      ...
      <aside class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm xl:col-span-3">
        ...
        <div class="mt-5">
          ...
          <div class="mt-3 grid gap-2">
            <div v-for="item in tagStats" :key="item.tag"
              ...
              <span class="rounded-full bg-blue-100 px-2 py-0.5 text-xs font-semibold text-blue-700">{{ item.count
                }}</span>
            </div>
          </div>
        </div>
    ...
    <section class="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div class="mb-5 flex flex-wrap items-end justify-between gap-3">
        ...
        <div class="flex flex-wrap gap-2">
          <input v-model.trim="search" type="text" placeholder="Tìm theo tên hoặc mô tả..."
        ...
        <div
          ...
          <div class="flex flex-wrap items-center gap-2">
            ...
            <button v-for="page in totalPages" :key="page" type="button"
              class="h-9 w-9 rounded-full border text-xs font-semibold transition" :class="page === currentPage
...
useHead({
  title: "Admin Dashboard - bintech 3D",
  meta: [{ name: "description", content: "Dashboard quản trị sản phẩm bintech 3D" }]
})
...
onMounted(() => {
  if (import.meta.client) {
    isAuthed.value = window.localStorage.getItem("admin_auth_status") === "true"
    if (isAuthed.value) {
      fetchProducts(true)
    }
  }
...
// Xóa onMounted cũ vì đã gộp lên trên
</script>
